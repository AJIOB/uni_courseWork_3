...
bool AJIOB_BinaryFileInputOutput::FileOpenIn()
{
	...
	try
	{
		...
		if (!fin.is_open())
		{
			AJIOB_ProgramLog("Не удается открыть файл " + cl_fileName + " для чтения");
			return false;
		}
	}
	catch (std::ios_base::failure e)
	{
		AJIOB_ProgramLog("Не удается открыть файл " + cl_fileName + " для чтения. Ошибка: " + e.what());
		return false;
	}
...
bool AJIOB_BinaryFileInputOutput::FileOpenOut()
{
	...
	try
	{
		...
		if (!fout.is_open())
		{
			AJIOB_ProgramLog("Не удается открыть файл " + cl_fileName + " для записи");
			return false;
		}
	}
	catch (std::ios_base::failure e)
	{
		AJIOB_ProgramLog("Не удается открыть файл " + cl_fileName + " для записи. Ошибка: " + e.what());
		return false;
	}
